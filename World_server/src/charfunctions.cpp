...
bool CCharacter::CanAttack( )
{
    ...
    if( (unsigned int)etime < (CLOCKS_PER_SEC * 100 / Stats->Attack_Speed) ) return false;
    return true;
}
...
void CCharacter::RefreshBuff( )
{
    ...
    for( UINT i=0;i<30;i++)
    {
        ...
        if( (unsigned int)etime >= MagicStatus[i].Duration * CLOCKS_PER_SEC )
        {
