...
void CWorldServer::pakPlayer( CPlayer *thisclient )
{
    ...
    for (unsigned int i=0; i<30; i++)//unused ????
        ADDWORD( pak, 0x00 );
...
bool CWorldServer::pakDoDrop( CPlayer* thisclient, CPacket* P )
{
    ...
    if (itemid == 0)
    {
        ...
        if ( (unsigned int)thisclient->CharInfo->Zulies < amount ) return true;
...
bool CWorldServer::pakGate( CPlayer* thisclient, CPacket* P )
{
	...
	Log(MSG_INFO, "Gate %f %f", thisgate->src.x, thisgate->src.y);
...
bool  CWorldServer::pakLevelUpSkill( CPlayer *thisclient, CPacket* P )
{
    ...
    if ((unsigned int)thisclient->CharInfo->SkillPoints < nextskill->sp)
    {
    ...
    if ((unsigned int)thisclient->CharInfo->Zulies < nextskill->zuly)
    {
        BEGINPACKET( pak, 0x7b1 );
        ADDBYTE( pak, 0x01 );
        thisclient->client->SendPacket( &pak );
    ...
    for (unsigned int i = 0; i < 2; i++)
    {
        ...
        {
        case sLevel:
            if ((int)nextskill->reqam[i] > thisclient->Stats->Level) fail = true;
            break;
        case sStrength:
...
bool CWorldServer::pakCraft( CPlayer* thisclient, CPacket* P )
{
    ...
    if (newslot !=0xffff)
    {
        ...
        int	m = 0;
        for (unsigned int used=4; used != 12; used +=2)
        {
...
bool CWorldServer::pakChangeStorage( CPlayer* thisclient, CPacket* P)
{
    ...
    */
    ...
    case 0x00: //Deposit
    {
        ...
        unsigned int count = clientitem.count;
        ...
        if ((unsigned int)thisclient->CharInfo->Zulies >=bfee){
...
bool CWorldServer::pakStoreZuly( CPlayer* thisclient, CPacket* P)
{
    ...
    case 0x10://deposit'
    {
        if (zuly > (unsigned int)thisclient->CharInfo->Zulies)
            return true;
    ...
    case 0x11://withdraw
    {
        if (zuly > (unsigned int)thisclient->CharInfo->Storage_Zulies )
            return true;
        thisclient->CharInfo->Zulies += zuly;
        thisclient->CharInfo->Storage_Zulies -= zuly;
        BEGINPACKET( pak, 0x7da );
...
bool CWorldServer::pakBuyShop( CPlayer* thisclient, CPacket* P )
{
    ...
    case 0x01://check this
    {
        ...
        if ( count > otherclient->Shop->SellingList[slot].count )
            return true;
        newitem.count = count;
        if ( (unsigned int)thisclient->CharInfo->Zulies<(otherclient->Shop->SellingList[slot].price*count) )
            return true;
...
bool CWorldServer::pakSellShop( CPlayer* thisclient, CPacket* P )
{
    ...
    case 0x01:
    {
        ...
        if (thisclient->items[invslot].itemtype>9 && thisclient->items[invslot].itemtype<14)
            ...
            count = 1;
        newitem.count = count;
        if ( (unsigned int)otherclient->CharInfo->Zulies<(otherclient->Shop->BuyingList[slot].price*count) )
            return true;
...
//     }
    ...
    case 0x03: //itemdivide by npc
    {
        ...
        {
            if ((unsigned int)STB_ITEM[11].rows[i][4]==ProductList.Index[material_lookup_number]->item[0])
            {
        ...
        for (unsigned int teller = 1; teller<4;teller++)
        {
    ...
    case 0x05://Player Refine
    {
        ...
        unsigned int refinechance = upgrade[nextlevel];
