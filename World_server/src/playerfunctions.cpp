...
bool CPlayer::GetPlayerInfo( )
{
	...
	ADDBYTE( pak, 0 );
	client->SendPacket(&pak);
    sprintf(text,"HP: %i/%i , MP: %i/%i",(int)Stats->HP,Stats->MaxHP,(int)Stats->MP,Stats->MaxMP);
	RESETPACKET( pak, 0x0784 );
	ADDSTRING( pak, "[GM]PlayerInfo" );
	ADDBYTE( pak, 0 );
	ADDSTRING( pak, text );
...
{
    if ((long)Stats->MaxHP==Stats->HP && (long)Stats->MaxMP== Stats->MP)
    {
       lastRegenTime=0;
       return true;
    }
    ...
    if( etime >= 8 * CLOCKS_PER_SEC && Stats->HP > 0 )
    {
        ...
        Stats->HP += hpamount;
        Stats->MP += mpamount;
        if( Stats->HP > (long)Stats->MaxHP)
            Stats->HP = Stats->MaxHP;
        if( Stats->MP > (long)Stats->MaxMP )
...
bool CPlayer::PlayerHeal()
{
    ...
    if( UsedItem->usevalue!=0 && transtime >= 0.3*CLOCKS_PER_SEC )
    {
        ...
        {
            unsigned int value = UsedItem->userate;
            ...
            switch( UsedItem->usetype )
            {
                case 16: // HP
                    Stats->HP += value;
                    if(Stats->HP > (long)GetMaxHP())
                        Stats->HP = GetMaxHP();
                break;
                case 17: // MP
                    Stats->MP += value;
                    if(Stats->MP > (long)GetMaxMP())
                        Stats->MP = GetMaxMP();
                break;
