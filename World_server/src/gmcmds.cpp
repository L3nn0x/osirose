...
bool CWorldServer::pakGMCommand( CPlayer* thisclient, CPacket* P )
{
    ...
    else if (strcmp(command, "mystat")==0)
    {
        ...
        {
            char buffer2[200];
            sprintf ( buffer2, "My current HP is %i", (int)thisclient->Stats->HP );
            SendPM(thisclient, buffer2);
        }
        else if (strcmp(tmp, "mp")==0)
        {
            char buffer2[200];
            sprintf ( buffer2, "My current MP is %i", (int)thisclient->Stats->MP );
            SendPM(thisclient, buffer2);
        }
    ...
    else if (strcmp(command, "go")==0) // Use SQL by Likol
    {
        ...
        else
        {
            ...
            if ( thisclient->Stats->HP < (long)(thisclient->Stats->MaxHP / 2) || thisclient->Stats->HP < 1 || thisclient->Session->inGame == false )
            {
...
//         delete []update;
    ...
    else if (strcmp(command, "b")==0)
    {
        ...
        else
        {
            ...
            char buffer2[200];
            sprintf ( buffer2, "Please wait %i seconds before sending another global message.", (int)remaining );
            SendPM(thisclient, buffer2);
        }
        return true;
    }
...
bool CWorldServer::ReloadMobSpawn( CPlayer* thisclient, int id )
{
    ...
    CMap* map = MapList.Index[thisspawn->map];
    for (unsigned int j=0; j<thisspawn->max; j++)
    {
...
bool CWorldServer::pakGMInfo(CPlayer* thisclient, char* name)
{
    ...
    if (otherclient==NULL)
        return true;
    char  buffer[200];
	char	names[][32] = {"Visitor", "Soldier", "Knight", "Champion",
							"Muse", "Mage", "Cleric", "Hawker", "Raider",
							"Scout", "Dealer", "Bourgeois", "Artisan", "Unknown"};
    char* jobname;
...
bool CWorldServer::pakGMChangeFairyWait(CPlayer* thisclient, int newvalue)
{
    ...
    for (unsigned int i=0; i<GServer->FairyList.size(); i++)
    {
...
bool CWorldServer::GMShowTargetInfo( CPlayer* thisclient )
{
    ...
    ADDBYTE( pak, 0 );
    thisclient->client->SendPacket( &pak );
    sprintf( buffer, "Target HP/MAXHP: %i/%i", (unsigned int)monster->Stats->HP, monster->Stats->MaxHP );
    RESETPACKET( pak, 0x784 );
    ADDSTRING  ( pak, "[SYS]TargetInfo" );
    ADDBYTE    ( pak, 0 );
    ADDSTRING  ( pak, buffer );
