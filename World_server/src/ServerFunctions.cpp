...
{   //if droptype = 1 then it is a normal drop. if it is 2 then it is a potential side drop.
    ...
    if( thismon->IsGhost())
    {
        ...
        {
            for(unsigned int i=0; i<SkillbookList.size( ); i++)
            {
                newdrop->type = 2;
    ...
    {
        int dropmode = 0;
        unsigned int randv = RandNumber( 1, 100);
        ...
        for(unsigned int i=0; i<MDropList.size( ); i++)
        {
            isdrop=false;
            ...
            if(isdrop == true)
            {
                ...
                {
                    if(itemtype[n] != (int)droptype)continue;
                }
...
void CWorldServer::RefreshFairy( )
{
        ...
        if (Config.FairyMode == 1 && ClientList.size() > 1){   //if fairy mode on and someone online
           ...
           {
                if ( FairyList.at(i)->LastTime <= (int)( clock() - (FairyList.at(i)->WaitTime*60*CLOCKS_PER_SEC) ) && !FairyList.at(i)->assigned )  // if fairy hour is now
	            {
